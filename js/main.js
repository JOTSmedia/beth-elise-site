...
(function() {
  ...
  function escapeHtml(str) {
    ...
    if (heroBgCanvas && heroAvatarCanvas) {
      ...
      function updateHeroLayoutTargets(force = false) {
        ...
        if (heroAvatarCanvas) {
          ...
          // [FIX] Calculate canvas CSS stretch factor. 
          // When mobile URL bar collapses, canvas CSS height stretches but logical buffer 'h' stays same (anti-thrash).
          // We must multiply target physical coordinates by the stretch factor so they render in the right visual spot!
          const scaleX = (w && cRect.width) ? (w / cRect.width) : 1;
          const scaleY = (h && cRect.height) ? (h / cRect.height) : 1;
          
          const heroLogo = cachedDOM.heroLogo;
          if (heroLogo) {
            const rect = heroLogo.getBoundingClientRect();
            if (rect.width > 0) {
              logoMoonX = (rect.left + rect.width * 0.5 - cRect.left) * scaleX;
              logoMoonY = (rect.top - cRect.top + (rect.height * 0.095)) * scaleY;
            }
          }
          ...
          if (pillBadge) {
            ...
            if (bRect.width > 0 && bRect.height > 0) {
              badgeCenterX = (bRect.left + bRect.width * 0.5 - cRect.left) * scaleX;
              const marginPad = Math.min(24, bRect.width * 0.12);
              badgeRightX = (bRect.left + bRect.width - marginPad - cRect.left) * scaleX;
              badgeLeftX = (bRect.left + marginPad - cRect.left) * scaleX;
              badgeTopY = (bRect.top - cRect.top - 10) * scaleY;
            }
          }
          ...
          if (bookBtn) {
            ...
            if (bbRect.width > 0) {
              bookBtnX = (bbRect.left + bbRect.width * 0.5 - cRect.left) * scaleX;
              bookBtnY = (bbRect.top - cRect.top - 8) * scaleY;
            }
          }
          ...
          if (aeyeWidget) {
            ...
            if (aeRect.width > 0) {
              ...
              const rawAeX = (aeRect.left + aeRect.width * 0.5 - cRect.left) * scaleX;
              const rawAeY = (aeRect.top + aeRect.height * 0.5 - cRect.top) * scaleY;
      ...
      function updateBethSpeechBubblePosition() {
        ...
        const isDualState = heroTinkerbell.state && (heroTinkerbell.state.startsWith('AURA_') || heroTinkerbell.state.startsWith('MENU_'));
        const rawTargetX = (isDualState && heroTinkerbell.fairyX !== undefined) ? heroTinkerbell.fairyX : heroTinkerbell.x;
        const rawTargetY = (isDualState && heroTinkerbell.fairyY !== undefined) ? heroTinkerbell.fairyY : heroTinkerbell.y;
        
        // [FIX] Convert internal canvas buffer coordinates back to physical screen coordinates
        // in case the canvas CSS is stretched (mobile URL bar collapse)
        const cRect = document.getElementById('hero-avatar-canvas').getBoundingClientRect();
        const invScaleX = (w && cRect.width) ? (cRect.width / w) : 1;
        const invScaleY = (h && cRect.height) ? (cRect.height / h) : 1;
        
        const targetX = cRect.left + (rawTargetX * invScaleX);
        const targetY = cRect.top + (rawTargetY * invScaleY);
        const isFacingLeft = (isDualState && heroTinkerbell.fairyFacingLeft !== undefined) ? heroTinkerbell.fairyFacingLeft : (heroTinkerbell.facingLeft !== undefined ? heroTinkerbell.facingLeft : true);
      ...
      function updateAndRenderHeroTinkerbell(ctx, now, dt = 0.016) {
        ...
        // 9. ASSISTANT RESTING INSIDE aEYE (DISAPPEARED until summoned)
          ...
          heroTinkerbell.isStrutting = false;
                ...
                // 10. COMPANION RACE TO MENU: LIVING aEYE & AVATAR BETH
          ...
          // Track live target for aEYE orb (#assistant-modal-icon-orb)
          let curTargetX = heroTinkerbell.targetX || (w * 0.5);
          let curTargetY = heroTinkerbell.targetY || Math.max(40, h * 0.20);
          const eyeOrb = document.getElementById('assistant-modal-icon-orb');
          if (eyeOrb) {
            const rect = eyeOrb.getBoundingClientRect();
          ...
          heroTinkerbell.x = (1 - easeP) * heroTinkerbell.startX + easeP * curTargetX + Math.sin(p * Math.PI) * 25;
          heroTinkerbell.y = (1 - easeP) * heroTinkerbell.startY + easeP * curTargetY + arc;
          heroTinkerbell.scale = 1.0;
          heroTinkerbell.alpha = 1;

          // Track live target for Avatar Beth (.assistant-modal-card top right)
          const card = document.querySelector('.assistant-modal-card');
          let fairyCornerX = (w * 0.5) + 190;
          let fairyCornerY = Math.max(40, h * 0.20) - 28;
          if (card) {
          ...
          // Eye looks around curiously during flight
          ...
          if (p >= 1) {
            ...
            emitPixieDust(heroTinkerbell.x, heroTinkerbell.y, 25, ['#00FFC8', '#FFD700', '#FFFFFF']);
            ...
            if (typeof showBethSpeechBubble === 'function') {
              showBethSpeechBubble("How may we help you?", heroTinkerbell.fairyX, heroTinkerbell.fairyY, 'auto');
            }
          }
        }

        // 11. MENU PERCHED: LIVING aEYE DOCKED AT HEADER ORB, BETH HOVERING AT CORNER
        else if (heroTinkerbell.state === 'MENU_PERCHED') {
          // Track live target for aEYE orb
          let curTargetX = heroTinkerbell.targetX || (w * 0.5);
          let curTargetY = heroTinkerbell.targetY || Math.max(40, h * 0.20);
          const eyeOrb = document.getElementById('assistant-modal-icon-orb');
          if (eyeOrb) {
          ...
          heroTinkerbell.x = curTargetX;
          heroTinkerbell.y = curTargetY;
          heroTinkerbell.scale = 1.0;
          heroTinkerbell.alpha = 1;

          // Track live target for Avatar Beth
          const card = document.querySelector('.assistant-modal-card');
          let fairyCornerX = (w * 0.5) + 190;
          let fairyCornerY = Math.max(40, h * 0.20) - 28;
          if (card) {
          ...
          heroTinkerbell.fairyCornerX = fairyCornerX;
          heroTinkerbell.fairyCornerY = fairyCornerY;
          heroTinkerbell.fairyX = fairyCornerX;
          heroTinkerbell.fairyY = fairyCornerY + Math.sin(now * 0.005) * 3.5;
          heroTinkerbell.fairyWingPhase = (heroTinkerbell.fairyWingPhase || 0) + dt * 14.0;
          heroTinkerbell.fairyFacingLeft = true;

          // Mouse gaze tracking for the living aEYE orb
          const mX = (typeof mouse !== 'undefined' && mouse.x !== undefined) ? mouse.x : (w * 0.5);
          const mY = (typeof mouse !== 'undefined' && mouse.y !== undefined) ? mouse.y : (h * 0.5);
          const dx = mX - heroTinkerbell.x;
          const dy = mY - heroTinkerbell.y;
          const dist = Math.hypot(dx, dy) || 1;
          const targetGazeX = Math.max(-0.45, Math.min(0.45, (dx / dist) * 0.45));
          const targetGazeY = Math.max(-0.35, Math.min(0.35, (dy / dist) * 0.35));
          heroTinkerbell.auraGazeX = (heroTinkerbell.auraGazeX || 0) + (targetGazeX - (heroTinkerbell.auraGazeX || 0)) * 0.12;
          heroTinkerbell.auraGazeY = (heroTinkerbell.auraGazeY || 0) + (targetGazeY - (heroTinkerbell.auraGazeY || 0)) * 0.12;
          ...
          if (Math.random() > 0.75) {
            emitPixieDust(
              heroTinkerbell.fairyX + (Math.random() - 0.5) * 18,
              heroTinkerbell.fairyY + (Math.random() - 0.5) * 18,
              1,
              ['#FFD700', '#00FFC8', '#FFF']
            );
          }
        ...
        // 12. MENU DIVE BACK: aEYE & AVATAR BETH DIVE BACK TO HOMEBASE
          ...
          // Fairy Avatar Beth dives back to homebase alongside aEye
        ...
        if (heroTinkerbell.state !== 'SPAWNING') {
          ...
          // 3. AURA Scanner & Assistant Modal Flying aEYE Companion
          else if (
            heroTinkerbell.state === 'AURA_TAKEOFF' || heroTinkerbell.state === 'AURA_SCANNING' || heroTinkerbell.state === 'AURA_HOVER' || heroTinkerbell.state === 'AURA_DIVE_BACK' || heroTinkerbell.state === 'AURA_RESCAN_BLINK' || heroTinkerbell.state === 'AURA_LOCKED' ||
            heroTinkerbell.state === 'MENU_TAKEOFF' || heroTinkerbell.state === 'MENU_PERCHED' || heroTinkerbell.state === 'MENU_DIVE_BACK'
          ) {
            ...
            // 2. Cosmic Housing Outer Bevel (Gold Filigree Ring)
            const housingGrad = ctx.createLinearGradient(-eyeRad, -eyeRad, eyeRad, eyeRad);
            housingGrad.addColorStop(0, '#FFE875');
            housingGrad.addColorStop(0.35, '#E5A93B');
            housingGrad.addColorStop(0.70, '#9D4EDD');
            housingGrad.addColorStop(1, '#00FFC8');

            ctx.strokeStyle = housingGrad;
            ctx.lineWidth = Math.max(1.8, eyeRad * 0.08);
            ctx.beginPath();
            ctx.arc(0, 0, eyeRad, 0, Math.PI * 2);
            ctx.stroke();

            // 3. Dark Cosmic Indigo Body
            const bodyGrad = ctx.createRadialGradient(0, 0, eyeRad * 0.1, 0, 0, eyeRad);
            bodyGrad.addColorStop(0, 'rgba(36, 11, 74, 0.98)');
            bodyGrad.addColorStop(0.75, 'rgba(14, 2, 30, 0.98)');
            bodyGrad.addColorStop(1, 'rgba(4, 0, 10, 0.98)');
            ctx.fillStyle = bodyGrad;
            ctx.beginPath();
            ctx.arc(0, 0, eyeRad * 0.94, 0, Math.PI * 2);
            ctx.fill();

            // 4. Sacred Filigree Dots Orbiting Rim
            const dotCount = 12;
            const dotRadius = eyeRad * 0.84;
            const dotAngleOffset = now * 0.0012;
            for (let i = 0; i < dotCount; i++) {
              const ang = dotAngleOffset + (i * Math.PI * 2) / dotCount;
              const dX = Math.cos(ang) * dotRadius;
              const dY = Math.sin(ang) * dotRadius;
              ctx.fillStyle = i % 2 === 0 ? 'rgba(255, 215, 0, 0.85)' : 'rgba(0, 255, 200, 0.75)';
              ctx.beginPath();
              ctx.arc(dX, dY, Math.max(1.2, eyeRad * 0.038), 0, Math.PI * 2);
              ctx.fill();
            }

            // 5. Living Sclera & Iris Eye inside the orb
            const pupilR = eyeRad * 0.32;
            const eyeColors = ['#7B2CBF', '#9D4EDD', '#00FFC8'];
            if (typeof drawOrganicEye === 'function') {
              drawOrganicEye(ctx, 0, 0, eyeRad * 0.88, heroTinkerbell.auraGazeX || 0, heroTinkerbell.auraGazeY || 0, heroTinkerbell.auraBlinkPhase || 0, pupilR, eyeColors, now);
            }

            // 6. Specular Glass Arc Highlight
            ctx.strokeStyle = 'rgba(255, 255, 255, 0.85)';
            ctx.lineWidth = Math.max(1.4, eyeRad * 0.05);
            ctx.lineCap = 'round';
            ctx.beginPath();
            ctx.arc(0, 0, eyeRad * 0.72, -Math.PI * 0.75, -Math.PI * 0.25);
            ctx.stroke();
            ...
            // ─── RENDER AVATAR BETH PERCHED ON CONTAINER CORNER (EXACT SAME SHARED LOGIC FOR AURA & MENU) ───
            if (heroTinkerbell.fairyX !== undefined && heroTinkerbell.fairyY !== undefined) {
              const isPerchedState = (
                heroTinkerbell.state === 'AURA_SCANNING' ||
                heroTinkerbell.state === 'AURA_LOCKED' ||
                heroTinkerbell.state === 'AURA_HOVER' ||
                heroTinkerbell.state === 'MENU_PERCHED'
              ...
              drawHeroTinkerbellSprite(
                ...
                -0.18, // head angled curiously down at the card/portal
                isPerchedState, // gentle perched stance
                0,
                false,
                0,
      ...
      window.triggerFairyMenuTakeoff = function() {
        ...
        // Target for Living aEYE: Top-center orb dais of assistant modal
        ...
        // Target for Avatar Beth: Top-right corner of assistant modal card
        const card = document.querySelector('.assistant-modal-card');
        let fairyCornerX = (window.innerWidth * 0.5) + 190;
        let fairyCornerY = Math.max(40, window.innerHeight * 0.20) - 28;
        if (card) {
          const cr = card.getBoundingClientRect();
          if (cr.width > 0 && cr.height > 0) {
            fairyCornerX = cr.right - 14;
        ...
        heroTinkerbell.targetX = menuEyeX;
        heroTinkerbell.targetY = menuEyeY;
        heroTinkerbell.scale = 1.0;
        heroTinkerbell.alpha = 1;

        heroTinkerbell.fairyCornerX = fairyCornerX;
        heroTinkerbell.fairyCornerY = fairyCornerY;
        heroTinkerbell.fairyStartX = curAeyeX;
        heroTinkerbell.fairyStartY = curAeyeY;
        heroTinkerbell.fairyX = curAeyeX;
        heroTinkerbell.fairyY = curAeyeY;
        heroTinkerbell.fairyWingPhase = 0;
        heroTinkerbell.fairyFacingLeft = true;
        heroTinkerbell.menuBubbleShown = false;

        // Ensure canvas layer is visible for flight
        if (heroAvatarCanvas) {
        ...
        emitPixieDust(curAeyeX, curAeyeY, 35, ['#FFD700', '#00FFC8', '#FFFFFF', '#C77DFF']);
      ...
      window.triggerFairyMenuDiveBack = function() {
        ...
        heroTinkerbell.progress = 0;
        heroTinkerbell.startX = heroTinkerbell.x || (window.innerWidth * 0.5);
        heroTinkerbell.startY = heroTinkerbell.y || Math.max(40, window.innerHeight * 0.20);
        heroTinkerbell.scale = 1.0;
        heroTinkerbell.alpha = 1;
    ...
    // Initial check
    updateActiveNav();
    // [FIX] Ensure positions are perfectly aligned once fonts, images, and layout fully settle on mobile
    window.addEventListener('load', () => {
      // Force scroll to top on load to defeat browser scroll memory or URL bar auto-scroll
      window.scrollTo(0, 0);
      setTimeout(() => window.scrollTo(0, 0), 10);
      setTimeout(() => window.scrollTo(0, 0), 50);
      if (typeof resize === 'function') resize();
      if (typeof updateHeroLayoutTargets === 'function') updateHeroLayoutTargets(true);
      if (typeof updateBethSpeechBubblePosition === 'function') updateBethSpeechBubblePosition();
    }, { once: true });
