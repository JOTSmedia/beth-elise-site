  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    if (preloader) {
      ...
      const dismissPreloader = () => {
        ...
        // Start the celestial woman in the Glinda orb descent right as preloader clears
    ...
    if (heroBgCanvas && heroAvatarCanvas) {
      ...
      window.triggerHeroDescent = function() {
        ...
        const greetReset = document.getElementById('beth-greeting-bubble');
        if (greetReset) { greetReset.classList.remove('is-visible'); greetReset.hidden = true; }
      };
      ...
      function updateAndRenderHeroTinkerbell(ctx, now, dt = 0.016) {
        ...
        if (pillBadge && heroAvatarCanvas) {
          ...
          if (bRect.width > 0 && bRect.height > 0) {
            ...
            // She walks along the top edge of the tagline pill.
        ...
        // 1. SPAWNING → Begin Glinda starlight bubble orb descent from star
        ...
        // 2. ORB FLOATING — Dreamy Glinda the Good Witch Starlight Bubble Orb Descent (~7.5s)
          ...
          if (oft >= orbDuration) {
            // Smooth arrival directly onto the top of the crescent moon
        ...
        // 4. PERCHED ON TOP OF CRESCENT MOON — Badass Look-Around (~3.2s)
          ...
          // Moon crescent glow under boots
          ctx.save();
          const moonGlowPulse = 1.0 + Math.sin(now * 0.004) * 0.18;
          const moonGlowGrad = ctx.createRadialGradient(heroTinkerbell.x, heroTinkerbell.y + 14, 0, heroTinkerbell.x, heroTinkerbell.y + 14, 32 * moonGlowPulse);
          moonGlowGrad.addColorStop(0, 'rgba(255, 255, 255, 0.98)');
          moonGlowGrad.addColorStop(0.35, 'rgba(255, 215, 0, 0.85)');
          moonGlowGrad.addColorStop(0.65, 'rgba(0, 229, 212, 0.45)');
          moonGlowGrad.addColorStop(1, 'transparent');
          ctx.fillStyle = moonGlowGrad;
          ctx.beginPath();
          ctx.arc(heroTinkerbell.x, heroTinkerbell.y + 14, 32 * moonGlowPulse, 0, Math.PI * 2);
          ctx.fill();
          ctx.restore();
        ...
        // 5b. PERCHED ON BOOK BUTTON (brief pause ~1.4s)
        ...
        // 7. CONFIDENT BADASS CATWALK STRUT ACROSS PILL BAR (Replaces worm dance ~4.2s)
          ...
          // The source pose leads with her left foot toward screen-left, but the
          // strut travels left→right — unmirrored she moonwalks. Mirroring here
          // turns her to face the way she is actually going.
          ...
          if (st >= totalStrutDuration) {
            // Reached right edge! Turn and launch into Olympic somersault dive into aEye!
        ...
        // 8. OLYMPIC DIVE — Acrobatic somersault plunge directly into aEye assistant (~2.2s)
          ...
          // Upward launch arc then steep graceful dive into aEye
          ...
          // Pixie-dust trail. Emitting only at the current position leaves gaps
          // between frames at dive speed, so the dust is laid down along the
          // segment she just travelled — that is what makes it read as a ribbon
          // streaming behind her rather than a few loose sparkles.
          const TRAIL_STEPS = 5;
          ...
          if (p >= 1) {
            // She lands and greets the visitor first; the aEYE handoff happens
            // at the end of BETH_GREETING below.
            heroTinkerbell.state = 'BETH_GREETING';
            heroTinkerbell.greetTime = 0;
            heroTinkerbell.diveAngle = 0;
            heroTinkerbell.alpha = 1;
            emitPixieDust(aeyeX, aeyeY, 34, ['#C77DFF', '#00FFC8', '#FFD700', '#FFFFFF']);

            heroTinkerbell.greetShown = false;   // bubble waits until she settles
            if (window.celestialAudio) window.celestialAudio.playChime(639, 1.0);
          }
        }

        // 8b. BETH GREETS, then hands over to the aEYE
        else if (heroTinkerbell.state === 'BETH_GREETING') {
          heroTinkerbell.greetTime += dt;
          heroTinkerbell.wingPhase += dt * 11.0;
          heroTinkerbell.isStrutting = false;
          heroTinkerbell.diveAngle = 0;
          heroTinkerbell.facingLeft = false;
          // hover just above the eye so she and the bubble both read
          heroTinkerbell.x = aeyeX;
          heroTinkerbell.y = aeyeY - 40;
          heroTinkerbell.bodySway = Math.sin(now * 0.004) * 1.6;

          // 0 → SETTLE: she has just landed, no bubble yet.
          // SETTLE → GREET_TIME - CLEAR: she speaks.
          // last CLEAR seconds: bubble is gone, so it is off screen before she jumps.
          const SETTLE = 0.4;
          const CLEAR = 0.55;
          const GREET_TIME = 3.4;
          heroTinkerbell.alpha = 1;
          ...
          if (gEl) {
            ...
            if (!heroTinkerbell.greetShown && gt >= SETTLE && gt < GREET_TIME - CLEAR) {
              ...
              // Anchor the tail to her mouth. In the source render her mouth sits
              // at about y=175 of 1024; the sprite draws from -374 with its origin
              // at her ankles, so that lands 36px above the origin at this scale.
              // The aEYE button is position:fixed, so its rect is already in
              // viewport space — no canvas-to-viewport conversion needed.
              const abtn = document.getElementById('assistant-avatar-btn');
              const abr = abtn.getBoundingClientRect();
              const mouthX = abr.left + abr.width / 2;
              const mouthY = abr.top + abr.height / 2 - 40 - 36;
              gEl.hidden = false;
              gEl.style.visibility = 'hidden';
              requestAnimationFrame(() => {
                const r = gEl.getBoundingClientRect();
                // tail centre sits 36px in from the bubble's right edge (26 + 10)
                let left = mouthX + 36 - r.width;
                let top = mouthY - 16 - r.height;
          ...
          if (Math.random() > 0.55) {
            emitPixieDust(heroTinkerbell.x + (Math.random() - 0.5) * 22,
                          heroTinkerbell.y + (Math.random() - 0.5) * 22,
                          1, ['#C77DFF', '#FFD700', '#FFF']);
          }

          if (heroTinkerbell.greetTime >= GREET_TIME) {
            heroTinkerbell.state = 'BETH_JUMP';
        ...
        // 8c. BETH JUMPS INTO THE APPEARING aEYE
        else if (heroTinkerbell.state === 'BETH_JUMP') {
          heroTinkerbell.jumpTime += dt;
          heroTinkerbell.isStrutting = false;
          heroTinkerbell.facingLeft = false;
          heroTinkerbell.diveAngle = 0;

          const CROUCH = 0.30;        // she loads up before pushing off
          const FLIGHT = 0.85;        // then the arc into the eye
          const jt = heroTinkerbell.jumpTime;

          if (jt < CROUCH) {
            // dip down and compress, wings beating faster as she loads
            const c = jt / CROUCH;
            heroTinkerbell.wingPhase += dt * 16.0;
            heroTinkerbell.y = heroTinkerbell.startY + Math.sin(c * Math.PI) * 9;
            heroTinkerbell.jumpSquash = 1 + Math.sin(c * Math.PI) * 0.10;
            heroTinkerbell.alpha = 1;
          } else {
            const f = Math.min(1, (jt - CROUCH) / FLIGHT);
            heroTinkerbell.wingPhase += dt * 30.0;
            heroTinkerbell.jumpSquash = 1 - Math.sin(f * Math.PI) * 0.06;   // stretch in the air

            // arc: up and over, then down into the eye
            const arc = Math.sin(f * Math.PI) * 78;
            heroTinkerbell.x = heroTinkerbell.startX + (aeyeX - heroTinkerbell.startX) * f;
            heroTinkerbell.y = heroTinkerbell.startY + (aeyeY - heroTinkerbell.startY) * f - arc;
            heroTinkerbell.diveAngle = f * Math.PI * 0.9;

            // trailing sparks off her heels
            for (let t = 0; t < 3; t++) {
              emitPixieDust(heroTinkerbell.x + (Math.random() - 0.5) * 10,
                            heroTinkerbell.y + 6 + (Math.random() - 0.5) * 10,
                            1, ['#C77DFF', '#FFD700', '#00FFC8', '#FFF']);
            }

            // the eye materialises just before she reaches it, so she lands INTO it
            if (f > 0.55 && !heroTinkerbell.eyeSummoned) {
              heroTinkerbell.eyeSummoned = true;
              if (typeof window.activateSacredAssistantWidget === 'function') {
                window.activateSacredAssistantWidget();
              }
            }

            // she shrinks into the eye over the last third of the arc
            heroTinkerbell.alpha = f > 0.7 ? Math.max(0, (1 - f) / 0.3) : 1;

            if (f >= 1) {
            // She dissolves into the eye — starburst, then the aEYE takes over.
        ...
        if (heroTinkerbell.state !== 'SPAWNING') {
          ...
          // 2. Materializing / Popping into stardust
          else if (heroTinkerbell.state === 'FLYING_TO_LOGO') {
            const p = Math.min(1, heroTinkerbell.progress);
            ctx.save();
            if (p < 0.5) {
      ...
      imgAvatar.src = 'images/avatar_body.png?v=37.0';

      const imgWingL = new Image();
      imgWingL.src = 'images/avatar_wing_left.png?v=37.0';

      const imgWingR = new Image();
      imgWingR.src = 'images/avatar_wing_right.png?v=37.0';

      const imgTrain = new Image();
      imgTrain.src = 'images/avatar_train.png?v=38.0';
      ...
      function drawLeg(ctx, hipSrc, phase, tone, shoeFill) {
        ...
        // Heel + pointed toe. The ankle only takes part of the shin's angle,
        // otherwise the toe swings far past anything a foot does.
      ...
      /**
       * Swing the dress train. It hangs off her hip on its own layer, and lags
       * behind the stride the way heavy fabric does — the peak of the swing
       * arrives after the hip that drove it. Left static it read as a tail.
       */
       ...
       * hinge while being foreshortened horizontally, which is what sells the
       * wing turning through the air rather than merely rocking.
       */
      function beatWing(ctx, img, pivotSrc, beat, mirrored) {
        if (!img.complete || !img.naturalWidth) return;
        const p = avLocal(pivotSrc);
        const dir = mirrored ? -1 : 1;
        const tilt = dir * (-0.13 + 0.26 * (0.5 + 0.5 * beat));
        const spread = 0.80 + 0.28 * (0.5 + 0.5 * beat);
      ...
      imgGlindaOrb.src = 'images/photorealistic_glinda_bubble_orb.png?v=22.0';
      ...
      function drawCelestialOrb(ctx, x, y, now, opacity = 1.0) {
        ...
        const pulse = 1.0 + Math.sin(now * 0.005) * 0.1;
        const orbSize = 58 * pulse;

        // Multi-tier Stardust Aura
        const aura = ctx.createRadialGradient(0, 0, 10, 0, 0, orbSize * 1.5);
        aura.addColorStop(0, 'rgba(255, 255, 255, 0.95)');
        aura.addColorStop(0.3, 'rgba(0, 255, 200, 0.6)');
        aura.addColorStop(0.65, 'rgba(199, 125, 255, 0.35)');
        ...
        // Draw Translucent 3D Avatar inside the Bubble.
        // The wings live on their own layers now, so they have to be drawn here
        // too or she rides down the screen wingless.
        if (imgAvatar.complete && imgAvatar.naturalWidth > 0) {
          ctx.save();
          ctx.globalAlpha = 0.9;
          const avH = orbSize * 0.9;
          const avW = avH;
          // gentle beat while she descends — both wings foreshorten together
          const curl = 0.84 + 0.16 * (0.5 + 0.5 * Math.sin(now * 0.012));
          [imgWingL, imgWingR].forEach(function (wing) {
            if (!wing.complete || !wing.naturalWidth) return;
            ctx.save();
            ctx.scale(curl, 1);
            ctx.drawImage(wing, -avW * 0.5, -avH * 0.55, avW, avH);
            ctx.restore();
          });

          // Her legs are drawn, not photographed, so they have to be drawn in
          // here as well or she rides down the bubble ending at the knees.
          // The sprite helpers work in ankle-origin space at AV_DRAW size, so
          // scale into that frame and re-anchor to the orb's top-left origin.
          ctx.save();
          const orbK = avW / AV_DRAW;
          ctx.scale(orbK, orbK);
          ctx.translate(0, (0.85 - 0.55) * AV_DRAW);
          drawLeg(ctx, HIP_BACK, Math.PI * 1.5 - 0.10, LEG_TONE_BACK, SHOE_BACK);
          drawLeg(ctx, HIP_FRONT, Math.PI * 0.5 + 0.10, LEG_TONE_FRONT, SHOE_FRONT);
          drawTrain(ctx, Math.PI * 0.5, false, now * 0.004);
          ctx.drawImage(imgAvatar, AV_OX, AV_OY, AV_DRAW, AV_DRAW);
          ctx.restore();

          ctx.restore();
        }

        // Draw Glinda Iridescent Bubble Orb Texture
        if (imgGlindaOrb.complete && imgGlindaOrb.naturalWidth > 0) {
          ...
          // Fallback iridescent bubble gradient
      ...
      // ─── 3D BADASS CELESTIAL FAIRY AVATAR SPRITE RENDERER ───
